...
Constraints
-----------
You can specify constraints for property. Each constraint has name and constraint logic - function. This function must
return `true` if constraint is satisfied and `false` otherwise. Constraint function is executed in context of clazz
or clazz instance depending of properties type. If setted value does not satisfy the constraint error will be thrown.
