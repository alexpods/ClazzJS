...
value of 'methods' option is ['get', 'set', 'has', 'is', 'remove', 'clear'] (as you can see bello this is 
all possible method types). It's using next logic for method name generation : 
```js
var methodName = methodTyp + propertyName[0].toUppersCase + propertyName.slice(1);
```
